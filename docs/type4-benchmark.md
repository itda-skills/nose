...
| proof fact | immutable binding, proven callee identity, table-key identity, static import/projection, nullish default, literal map-default fallback including Ruby zero-arg `fetch` blocks, ordered append-effect branch, ordered foreach-effect branch, ordered index-assignment branch, own-property guard, record-shape guard, equality-chain membership, flag+break reduction, ordered string-builder, statically-false loop guard, total-order comparator, C u16/u32 byte-pack, Java low-bit toggle, unsafe boundary |
