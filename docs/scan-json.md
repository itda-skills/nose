...
A checked-in example lives at [crates/nose-cli/tests/fixtures/scan-json-v1.json](../crates/nose-cli/tests/fixtures/scan-json-v1.json)
...
[query-JSON schema v7](query-json.md#semantic-packs); legacy scan JSON v1 does not
...
proof-only fragments, review-surface fragments, generated-header families, and CSS
source-plus-compiled/minified build pipelines. It is not raw detector output: families wholly in
...
filter `families[]` to `recommended_surface == "default"`. Do not drop a default family only
because one location has `looks_generated`: partly generated families can still be actionable
when the hand-written locations are the refactoring target. Treat `review`, `hidden`, and
`generated` as diagnostic surfaces: useful for audits, review-hazard tooling, and regression
checks, but not default refactoring recommendations.
...
| `shared_lines` | integer | Lines invariant across **all** copies (up to an 8-member cap), the all-copies anti-unification count — the same shared/removable headline `nose query` reports for the family. Earlier releases counted the representative pair, which over-stated families whose 3rd+ copies diverge. |
...
| `witness` | object, optional | The agent-facing equivalence witness: WHY the members merged. `kind` is `exact-value-graph` (every member strict-exact-safe with one identical value multiset; `value_nodes` carries its size — for proof substance see `semantic_laws` and fragment `reason_code`), `shared-sub-dag` (a common heavy anchor; see each location's `shared_subdag` span), `copy-paste-run` (token-identical contiguous run), or `structural-similarity` (the fuzzy near channel; `mean_value_jaccard` vs `mean_shape_jaccard` distinguish behaviorally-driven convergence from surface likeness). For enriched `structural-similarity` and `shared-sub-dag` families whose value DAGs align, the optional `graded` object grades representative copies into "equal except *k* holes" — see [graded-witness](graded-witness.md) and the row below. |
| `witness.graded` | object, optional | The anti-unification grade of an enriched near/shared-core family's representative copies whose value DAGs can be aligned: how nearly equal they really are, beyond the score or shared anchor. `holes` is *k* (the spots where the two value DAGs differ); `spots[]` itemizes them (each with a value `class` — `literal`/`input`/`field`/`call`/`lambda`/`operator`/`expr` are clean parameters, `arity`/`shape`/`unmodeled`/`extra-sink`/`decorator`/`async-mirror` are structural or transformation divergence — an `effect` flag, per-side `a_lines`/`b_lines`, and trimmed `a_text`/`b_text`). `equal_modulo_holes` is true when every hole is a small value leaf, the behavior sinks aligned, no name mismatched, no async/sync transformation is present, and same-language copies' decorators match — the strongest near-channel grade. `patterns[]` names recognized divergence shapes (`effects-reordered`, `sink-superset-a`/`-b`, `fragment-containment`, `low-substance`, `async-mirror`, `referent-mismatch`, `decorator-differs`). `referent_mismatches[]` lists names both copies consume that resolve to *different* definitions (which **demotes** the claim — fail-closed); `caveat_names[]` lists names unresolved on one side (the claim is scoped past them). `modeled_caveat` is true when either copy passed lossy lowering, so "equal" means equal in the modeled fraction. Scope: the unit *body* the value graph models plus a source-level decorator/attribute comparison when the selected representatives share a language (a `@deco(x)` vs `@deco(y)` difference fires `decorator-differs` and demotes the claim); the parameter *signature* is still not modeled (a differing unused parameter is invisible). Cross-language families can carry value-DAG grades when the graphs align, but remain non-source-comparable and skip decorator comparison. Absent for fragments and pathological files. Near-channel evidence, not an exact-channel proof. See [graded-witness](graded-witness.md). |
...
| `origin` | object, optional | Source-origin facets for this location when the frontend can prove them. This is evidence for hint rendering and future filters, not a verdict and not part of family ids or structured-ignore matching. Fields may include `domains` (`imperative`, `type-contract`, `implementation-type`, `data`, `style`, `markup`, `preprocessor`, …), `subkind` (`interface-trait-protocol`, `class`, `impl-block`, `css-rule`, `html-element`, `markup-fragment`, …), `body_kind` (`implementation`, `declaration-only`, `declarative-denotation`, `mixed`, …), `source_granularity`, `region_kind`, `container_kind` (`standalone-file`, `vue-sfc`, `svelte-component`, `html-document`, `jsx`, `tsx`), and `evidence_flags`. Markup flags such as `component-tag`, `bound-attributes`, and `static-attrs-only` are emitted only from the actual tag/attribute surface the frontend observed. Missing `origin` means unknown and falls back to legacy hints. |
...
| `looks_generated` | boolean, optional | Present (and `true`) when the file is recognized as generated or distributed output: a generated-code header (`Generated by …`, `@generated`, `Code generated … DO NOT EDIT`) or stylesheet distribution markers such as minified CSS, source-map references, preserved license/version banners, or sibling `.css.map` files. A family whose every location looks generated reports `recommended_surface: "generated"`; CSS build-pipeline families also report `generated` when one hand-written source rule is echoed into compiled/minified outputs. Other partly-generated families stay on their ranked surface with generated members flagged. |
