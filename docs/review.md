...
(`summary.divergences` / `summary.shown_divergences` on `nose query base=`, and
`total_inconsistent_families` / `shown_inconsistent_families` on deprecated `nose review`).
SARIF records `runs[].properties.total_families` and `shown_families`, plus a note pointing
to the full-output spelling (`top=0` for query, `--top 0` for deprecated review), so CI uploads
can detect when only the top findings were emitted.
