...
- [scheduling-channel-callback-obligations-594](scheduling-channel-callback-obligations-594.md) — issue #594's cross-language obligation vocabulary, Promise/scheduling closeout, and #602 census-backed guardrails plus the first exact `Promise.all` aggregate slice for scheduling, aggregate, cancellation, error/rejection channels, callback demand/effect, lifecycle, and mutation boundaries.
