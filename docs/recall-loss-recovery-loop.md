...
- [Java CompletableFuture constructor reporting](../bench/recall_loss/java-completablefuture-constructor-reporting-2026-07-02.v1.json) records the proof-backed constructor split from the broad Java Future bucket. Fully qualified or exact-/wildcard-import-backed `new CompletableFuture` calls now report future-settled, exception-channel, task-handle lifecycle, and cancellation/liveness obligations, newly aligning `46` occurrences while exact admission remains closed. The residual broad Java `CompletableFuture` bucket falls from `276` to `230`; the Java-heavy query regression kept all product output hashes identical and measured `7546.25ms -> 7684.07ms` (`+1.83%`).
