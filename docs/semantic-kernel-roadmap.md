...
The [promise-aggregate-raw-input-recovery-2026-06-29.v1.json](../bench/recall_loss/promise-aggregate-raw-input-recovery-2026-06-29.v1.json) artifact opens the shared raw-input subset for already-admitted literal `Promise.all` and `Promise.allSettled` aggregates. This strengthens the existing settlement capability instead of adding another selector rule: an aggregate element may recover as fulfilled either from Promise settlement evidence or from the same non-thenable-safe node/value proof used by `Promise.resolve`. The corpus scan records `8` `Promise.all` literal arrays and `1` `Promise.allSettled` literal array with a direct raw non-thenable element, with `3` fully lexical direct-safe candidates. Dynamic iterables, object/function raw inputs without non-thenable proof, untyped possible thenables, `Promise.race`, `Promise.any`, executor timing, and sync aggregate equivalence remain closed.
