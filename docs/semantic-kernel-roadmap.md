...
The [#657 hard-negative fixture artifact](../bench/recall_loss/issue-657-hard-negative-fixtures-2026-07-02.v1.json) adds a local executable guardrail suite instead of a new exact-admission slice. `crates/nose-cli/tests/equivalence/async_scheduling_hard_negatives.rs` contributes `8` test symbols and `42` fail-closed assertions across JS/TS, Go, Python, Rust, Java, Swift, and Ruby. It pins the capability boundaries named by #655: Promise receiver/channel/callback, executor, timer/microtask/cancellation, Go channel/select/goroutine/defer, Python asyncio/protocol, Rust Future-drive, Java Future/Executor/Stream, Swift task/async/try/continuation, and Ruby thread/fiber/yield/exception behavior. Remaining matrix classes are mapped to existing executable fixtures or checked reporting artifacts. Exact admission remains unchanged, so future behavior-changing PRs must extend this suite before opening broader async/scheduling convergence.
