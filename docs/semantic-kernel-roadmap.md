...
The follow-up [promise-async-function-return-recovery-2026-06-29.v1.json](../bench/recall_loss/promise-async-function-return-recovery-2026-06-29.v1.json) opens the narrow JS/TS same-file async-function producer slice behind that rule. The call-target producer derives `PromiseLike` result-domain evidence from direct callee identity plus JS/TS `Source::Protocol(AsyncFunction)` evidence, and the value graph uses the existing pure-inline sink fence before treating a non-thenable-safe returned payload as a fulfilled Promise state. This strengthens the semantic kernel by composing existing evidence capabilities instead of adding a selector-specific API. Non-JS async functions remain shared scheduling/protocol facts, not PromiseLike producer proof. `await`, throw/rejection paths, possible thenables, opaque call results, constructors, imported/member call returns, `.finally`, aggregate combinators, and sync payload equivalence remain closed.

2026-06-29 Promise direct-function return recovery note:
The follow-up [promise-direct-function-return-recovery-2026-06-29.v1.json](../bench/recall_loss/promise-direct-function-return-recovery-2026-06-29.v1.json) opens the proof-backed direct-function subset of the local/parameter call-return queue. The call-target producer now derives `PromiseLike` result-domain evidence from direct callee identity plus the returned expression's asserted `Domain(PromiseLike)` evidence, limited to non-async single-return functions. The value graph evaluates only that returned expression for Promise receiver recovery, so generic pure-inline global-binding policy stays unchanged. Literal and typed non-thenable `Promise.resolve` helper returns, plus `Promise.reject` helper returns, converge with direct Promise forms while remaining behind a Promise boundary. Parameter callees, member/imported call returns, unsafe thenables, constructors, `.finally`, aggregate combinators, and broad scheduling remain closed.
...
2026-06-29 Promise imported settled-value contract note:
The follow-up [promise-imported-settled-value-contract-2026-06-29.v1.json](../bench/recall_loss/promise-imported-settled-value-contract-2026-06-29.v1.json) adds the reusable `PromiseSettledValue` evidence capability for imported Promise producers. The semantic resolver admits a settled channel and payload only when a builtin, dependency-closed, unambiguous contract is anchored to the exact producer call, the same call has admitted `CallTarget::ImportedFunction` or `CallTarget::ImportedMember` identity, and the payload span/kind resolves to one IL node. Promise continuation recovery still requires `Domain(PromiseLike)` receiver proof and admitted `.then`/`.catch`/`.finally` API evidence; fulfilled payloads additionally require non-thenable-safe value proof. This lets contract-backed imported `load(1).then(...)` and rejected `load(1).catch(...)` fixtures recover while preserving Promise boundaries, but ordinary source-level imported producers, selector-only members, external/untrusted contracts, unsafe thenables, aggregate combinators, constructors, and broad scheduling remain closed.

2026-06-29 Node timers/promises domain recovery note:
The follow-up [promise-node-timers-domain-recovery-2026-06-29.v1.json](../bench/recall_loss/promise-node-timers-domain-recovery-2026-06-29.v1.json) opens only receiver-domain proof for the corpus-prioritized ESM named-import subset of Node `timers/promises`. The 120-repo JS/TS scan found `82` opened ESM call sites (`71` `setTimeout`, `11` `setImmediate`) across `execa`, `ky`, and `pixijs`; `15` CommonJS destructuring require call sites in `jest` remain closed. The new `LibraryApiContractId::JsImportedPromiseFactory` row materializes `Domain(PromiseLike)` for dependency-backed `node:timers/promises` or `timers/promises` imported bindings at documented arities, then the existing Promise receiver-method producer can admit `.then`. It intentionally emits no `PromiseSettledValue`, so timer delay/order, AbortSignal rejection, payload recovery, namespace/default imports, CommonJS requires, aggregate combinators, and broad scheduling equivalence remain closed.

2026-06-29 Node timers/promises CommonJS follow-up note:
The follow-up [promise-node-timers-commonjs-domain-recovery-2026-06-29.v1.json](../bench/recall_loss/promise-node-timers-commonjs-domain-recovery-2026-06-29.v1.json) opens the previously priced `15` CommonJS destructuring require call sites in `jest` as the same domain-only receiver proof, bringing the Node timers PromiseLike domain slice from `82` to `97` priced call sites. JS/TS lowering emits dependency-backed `ImportedBinding` evidence only for `const { setTimeout } = require("node:timers/promises")`, `const { setTimeout } = require("timers/promises")`, and the corresponding `setImmediate` aliases when `require` is unshadowed and the object pattern is fully static. Mutable `let`/`var`, computed/default/rest/nested patterns, shadowed `require`, namespace/default imports, unsupported arities, payload recovery, and broad scheduling equivalence remain closed.
...
The [promise-all-literal-aggregate-recovery-2026-06-29.v1.json](../bench/recall_loss/promise-all-literal-aggregate-recovery-2026-06-29.v1.json) artifact opens the first exact aggregate subset as a capability, not a selector shortcut. `Promise.all` now has a dependency-backed static-global contract, materializes `Domain(PromiseLike)`, and the value graph recovers only literal-array aggregates whose elements already prove fulfilled Promise state. The result stays behind a Promise boundary with an ordered sequence payload. Rejected elements, dynamic iterables, `Promise.race`, `Promise.any`, thenable assimilation, executor timing, and sync array equivalence remain closed. The pinned corpus has `397` broad `Promise.all` occurrences, `201` literal-array boundary occurrences, and `0` direct safe-seed occurrences for this narrow exact subset.

2026-06-29 #602 Promise.allSettled exact aggregate note:
The [promise-allsettled-literal-aggregate-recovery-2026-06-29.v1.json](../bench/recall_loss/promise-allsettled-literal-aggregate-recovery-2026-06-29.v1.json) artifact opens the next exact aggregate subset through the same aggregate contract kind, not a new selector-specific shortcut. `Promise.allSettled` now has dependency-backed static-global evidence, materializes `Domain(PromiseLike)`, and the value graph recovers only literal-array aggregates whose elements already prove fulfilled or rejected Promise state. The result stays behind a fulfilled Promise boundary with ordered settled-record payloads that keep fulfilled and rejected element channels distinct. Dynamic iterables, broad thenable assimilation, `Promise.all`, `Promise.race`, `Promise.any`, executor timing, and sync settled-record array equivalence remain closed. The pinned corpus has `17` broad `Promise.allSettled` occurrences, `8` literal-array boundary occurrences, and `0` direct safe-seed occurrences for this narrow exact subset.

2026-06-29 #602 Promise aggregate raw-input assimilation note:
The [promise-aggregate-raw-input-recovery-2026-06-29.v1.json](../bench/recall_loss/promise-aggregate-raw-input-recovery-2026-06-29.v1.json) artifact opens the shared raw-input subset for already-admitted literal `Promise.all` and `Promise.allSettled` aggregates. This strengthens the existing settlement capability instead of adding another selector rule: an aggregate element may recover as fulfilled either from Promise settlement evidence or from the same non-thenable-safe node/value proof used by `Promise.resolve`. The corpus scan records `8` `Promise.all` literal arrays and `1` `Promise.allSettled` literal array with a direct raw non-thenable element, with `3` fully lexical direct-safe candidates. At that slice, dynamic iterables, object/function raw inputs without non-thenable proof, untyped possible thenables, `Promise.race`, `Promise.any`, executor timing, and sync aggregate equivalence remained closed.

2026-06-30 #602 Promise.race/Promise.any exact aggregate note:
The [promise-race-any-literal-aggregate-recovery-2026-06-30.v1.json](../bench/recall_loss/promise-race-any-literal-aggregate-recovery-2026-06-30.v1.json) artifact opens the first-observed aggregate subset through the same dependency-backed aggregate contract kind. `Promise.race` now has static-global evidence and recovers only non-empty literal arrays whose elements all prove Promise settlement or non-thenable-safe raw fulfilled input; the value graph returns the first element's fulfilled/rejected settlement behind a Promise boundary. `Promise.any` uses the same closed input proof but recovers only when at least one element is fulfilled, returning the first fulfilled payload and leaving all-rejected AggregateError payloads closed. The pinned corpus has `32` broad `Promise.race` occurrences, `31` `Promise.race` literal arrays, `1` broad/literal `Promise.any` occurrence, and `0` fully closed lexical candidates for the opened paths. Dynamic iterables, possible thenables, all-rejected `Promise.any`, executor timing, cancellation/liveness, and sync value equivalence remain closed.

2026-06-30 #602 Promise executor readiness note:
The [promise-executor-boundary-audit-2026-06-30.v1.json](../bench/recall_loss/promise-executor-boundary-audit-2026-06-30.v1.json) artifact keeps `new Promise(...)` constructor recovery closed while pricing the next executor capability. The pinned corpus has `795` constructor occurrences across `18` repos. Most are not safe direct settlement candidates: `664` have side-effect/extra calls, `319` mention timer or scheduler constructs, `180` have multiple settlement calls, `338` resolved payload sites are possible thenable boundaries, and `9` contain `throw`. The lexical direct single-settlement upper bound is only `27` scalar resolves plus `4` scalar rejects. The next exact constructor slice must therefore be a capability admission, not a selector shortcut: unshadowed constructor proof, inline callback identity, single-settlement precedence, throw-to-rejection/throw-after-settlement ordering, callback effect reporting, non-thenable payload proof, and Promise-boundary preservation must all be explicit before behavior changes.

2026-06-30 #602 AbortSignal cancellation readiness note:
The [abort-signal-cancellation-boundary-audit-2026-06-30.v1.json](../bench/recall_loss/abort-signal-cancellation-boundary-audit-2026-06-30.v1.json) artifact keeps cancellation/liveness recovery closed while making the next capability measurable. Runtime-boundary diagnostics now split `AbortSignal.abort`, `AbortSignal.any`, `AbortSignal.timeout`, and `new AbortController()` into named cancellation and lifecycle obligations. The pinned corpus has `260` Abort mentions, `156` controller constructors, `175` `.abort()` selector calls, `323` `.signal` property reads, `193` signal option properties, `6` signal-bearing `fetch` calls, `2` signal-bearing timer calls, and `2` signal-bearing `addEventListener` calls. Exact cancellation admission remains closed with `semantic_admission_delta = 0`; a future exact slice must prove signal identity, abort ordering, abort reason propagation, listener/timer/fetch rejection behavior, and controller-signal lifecycle.

2026-06-30 #602 interval/scheduler lifecycle readiness note:
The [interval-scheduler-lifecycle-boundary-audit-2026-06-30.v1.json](../bench/recall_loss/interval-scheduler-lifecycle-boundary-audit-2026-06-30.v1.json) artifact keeps timer, interval, scheduler, and microtask recovery closed while making their capability requirements explicit. Runtime-boundary diagnostics now split global timer scheduling, `scheduler.wait` timing plus cancellation/liveness, `scheduler.yield` microtask ordering, `setInterval` repeated-emission lifecycle, interval cancellation, and one-shot timer/frame cancellation into named obligations. The pinned corpus has `780` `setTimeout` calls, `57` `setImmediate` calls, `73` bare `setInterval` calls, `55` `clearInterval` calls, `133` `clearTimeout` calls, `14` `queueMicrotask` calls, `43` `requestAnimationFrame` calls, and `11` `scheduler.yield` calls. Exact scheduling/lifecycle admission remains closed with `semantic_admission_delta = 0`; a future exact slice must prove callback identity, callback demand/effect, ordering, interval cardinality, and cancellation cleanup.
...
2026-07-02 #653 soundness hardening successor note:
The [#655 async/scheduling hard-negative matrix](../bench/recall_loss/issue-655-hard-negative-matrix-2026-07-02.v1.json) starts #653 as that successor guardrail epic, not as a behavior-changing exact-admission slice. It maps the currently reporting-supported and still-closed async/scheduling/lifecycle/error-channel families across JS/TS, Go, Swift, Rust, Java, Python, and Ruby before #657 adds executable fixture suites. The matrix audits `82` scoped lifecycle rows, including `65` reporting-supported closed-boundary rows, plus `11` supplemental JS/TS Promise continuation/rejection reporting and timer/scheduler priced surfaces; it names `48` missing hard-negative classes while keeping runnable fixture counts separate from reporting artifact evidence and preserving `semantic_admission_delta = 0`.

2026-07-02 #657 executable hard-negative fixture note:
The [#657 hard-negative fixture artifact](../bench/recall_loss/issue-657-hard-negative-fixtures-2026-07-02.v1.json) adds a local executable guardrail suite instead of a new exact-admission slice. `crates/nose-cli/tests/equivalence/async_scheduling_hard_negatives.rs` contributes `8` test symbols and `54` fail-closed assertions across JS/TS, Go, Python, Rust, Java, Swift, and Ruby. It pins the capability boundaries named by #655: Promise receiver/channel/callback, executor, timer/microtask/cancellation, Go channel/select/goroutine/defer, Python asyncio/protocol, Rust Future-drive, Java Future/Executor/Stream, Swift task/async/try/continuation, and Ruby thread/fiber/yield/exception behavior. The artifact maps all `48` matrix classes to explicit evidence status, with `40` classes covered by direct new executable evidence and, across the mapped statuses, `14` classes still flagged for more granular future executable follow-up. Exact admission remains unchanged, so future behavior-changing PRs must extend this suite before opening broader async/scheduling convergence.

2026-07-02 #654 semantic-kernel capability audit note:
The [#654 capability audit artifact](../bench/recall_loss/issue-654-semantic-kernel-capability-audit-2026-07-02.v1.json) checks the #653 vocabulary against the principle that kernel semantics should expose reusable capabilities, not an ever-growing feature list. It audits `15` capability groups, the current `12` `EvidenceKind` rows, `9` `SourceFactKind` families, `16` `SourceProtocolKind` rows, and `98` runtime-boundary obligation rules. The accepted shape keeps evidence records as facts, demand/effect as preconditions, source protocols as erased-syntax provenance, and obligation families as diagnostics-only exact-channel blockers. It also records `4` preserved legacy aliases, `6` duplicate/merge candidates, and `8` feature-shaped Promise diagnostics for #656 vocabulary cleanup. No public API or exact admission changes in this audit; the intended follow-up is documentation/label normalization while old checked artifacts remain readable.
...
statements and comprehensions emit an async iteration boundary with lifecycle,
value-channel, and scheduling obligations; `async with` emits an async context
boundary with lifecycle, cleanup, exception-channel, and scheduling
obligations. Exact admission remains closed with `false_merges = 0` and
