...
pub fn lower_corpus_filtered(roots: &[&Path], exclude: &[String]) -> Corpus {
    ...
    let mut files: Vec<Il> = paths
        ...
        .flat_map(|(i, (path, lang))| match std::fs::read(path) {
            Ok(src) => {
                if source_artifacts::skip_reason(Path::new(path), *lang, &src).is_some() {
                    Vec::new()
                } else {
                    lower_source_regions(FileId(i as u32), path, &src, *lang, &interner)
                }
            }
            Err(_) => Vec::new(),
        })
    ...
    fn lower_corpus_skips_obvious_cpp_headers_routed_as_c() {
        ...
        assert_eq!(
            source_artifacts::skip_reason(
                &c_header,
                Lang::C,
                fs::read(&c_header).unwrap().as_slice()
            ),
            None
        );
        assert_eq!(
