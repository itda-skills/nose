...
fn local_manifest_loads_as_metadata_only_opt_in() {
    ...
    assert_eq!(set.packs()[9].id, CSHARP_LANGUAGE_PACK_ID);
    assert_eq!(set.packs()[10].id, CSS_LANGUAGE_PACK_ID);
    assert_eq!(set.packs()[11].id, HTML_EMBEDDED_LANGUAGE_PACK_ID);
    assert_eq!(
        set.packs()[12].id,
        PYTHON_BUILTIN_COLLECTION_FACTORY_PACK_ID
    );
    assert_eq!(set.packs()[13].id, PYTHON_STDLIB_COLLECTION_FACTORY_PACK_ID);
    assert_eq!(set.packs()[14].id, PYTHON_STDLIB_MATH_PACK_ID);
    assert_eq!(set.packs()[15].id, RUBY_STDLIB_SET_PACK_ID);
    assert_eq!(set.packs()[16].id, RUST_STDLIB_VEC_PACK_ID);
    assert_eq!(set.packs()[17].id, RUST_STDLIB_OPTION_PACK_ID);
    assert_eq!(set.packs()[18].id, RUST_STDLIB_RESULT_PACK_ID);
    assert_eq!(set.packs()[19].id, RUST_STDLIB_INTEGER_METHOD_PACK_ID);
    assert_eq!(set.packs()[20].id, RUST_STDLIB_COLLECTION_FACTORY_PACK_ID);
    assert_eq!(set.packs()[21].id, RUST_STDLIB_MAP_FACTORY_PACK_ID);
    assert_eq!(set.packs()[22].id, SWIFT_STDLIB_COLLECTION_FACTORY_PACK_ID);
    assert_eq!(set.packs()[23].id, JAVA_STDLIB_MATH_PACK_ID);
    assert_eq!(set.packs()[24].id, JAVA_STDLIB_MAP_FACTORY_PACK_ID);
    assert_eq!(set.packs()[25].id, JAVA_STDLIB_MAP_ENTRY_PACK_ID);
    assert_eq!(set.packs()[26].id, JAVA_STDLIB_COLLECTION_FACTORY_PACK_ID);
    assert_eq!(
        set.packs()[27].id,
    ...
    assert_eq!(set.packs()[30].id, MAP_GET_PROTOCOL_PACK_ID);
    assert_eq!(set.packs()[31].id, MAP_GET_DEFAULT_PROTOCOL_PACK_ID);
    assert_eq!(set.packs()[32].id, FREE_FUNCTION_BUILTIN_PROTOCOL_PACK_ID);
    assert_eq!(set.packs()[33].id, PYTHON_ITERATOR_BUILTIN_PROTOCOL_PACK_ID);
    assert_eq!(set.packs()[34].id, RECEIVER_MEMBERSHIP_PROTOCOL_PACK_ID);
    assert_eq!(set.packs()[35].id, MAP_KEY_VIEW_PROTOCOL_PACK_ID);
    assert_eq!(set.packs()[36].id, PROPERTY_BUILTIN_PROTOCOL_PACK_ID);
    assert_eq!(set.packs()[37].id, BUILTIN_METHOD_CALL_PROTOCOL_PACK_ID);
    assert_eq!(set.packs()[38].id, STRING_AFFIX_PREDICATE_PROTOCOL_PACK_ID);
    assert_eq!(set.packs()[39].id, SEQUENCE_HOF_ADAPTER_PROTOCOL_PACK_ID);
    assert_eq!(set.packs()[40].id, GO_STDLIB_NAMESPACE_CALL_PACK_ID);
    assert_eq!(set.packs()[41].id, ITERATOR_IDENTITY_ADAPTER_PACK_ID);
    assert_eq!(set.packs()[42].id, JS_LIKE_BUILTIN_PROMISE_PACK_ID);
    assert_eq!(set.packs()[43].id, JS_LIKE_BUILTIN_ARRAY_PACK_ID);
    assert_eq!(set.packs()[44].id, JS_LIKE_BUILTIN_BOOLEAN_PACK_ID);
    assert_eq!(set.packs()[45].id, JS_LIKE_BUILTIN_REGEX_PACK_ID);
    assert_eq!(
        set.packs()[46].id,
    ...
    assert_eq!(set.packs()[48].id, PYTHON_STDLIB_TYPE_DOMAIN_PACK_ID);
    assert_eq!(set.packs()[49].id, VALUE_GRAPH_LAW_PACK_ID);
    let external = &set.packs()[50];
