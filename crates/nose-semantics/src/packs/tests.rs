...
fn local_manifest_loads_as_metadata_only_opt_in() {
    ...
    assert_eq!(set.packs()[12].id, PYTHON_STDLIB_TYPE_DOMAIN_PACK_ID);
    assert_eq!(set.packs()[13].id, FIRST_PARTY_VALUE_LAW_PACK_ID);
    let external = &set.packs()[14];
