...
2. `map_default_lookup`
   - why: Literal Python/Ruby, JS/TS literal `Map` construction, JS/TS object-literal own-property defaults, typed Go/Java/Rust, explicit typed TypeScript `Map` fallback lookups, and Java `Map.of`/`Map.ofEntries` literal factories are covered; imported/module-level construction, absent-key semantics, richer receiver proof facts, and mutation/effects remain open when receiver identity is not yet proven.
   ...
   - next probe: Continue with imported/module-level Map or object defaults only when receiver/key/default coordinates can be proven by static construction, import identity, immutable binding, or type facts beyond the current inline/local construction cases; keep arbitrary `.get`/index receivers, same-file type shadows, and prototype-aware `in` as hard boundaries.
