# Semantic-query regression harness

A repeatable harness for the **product** semantic query path, so detector changes
...
| file | what it is |
|---|---|
| `query_regression.py` | the harness (`baseline`, `compare`, `cache`, `selftest` subcommands) |
...
value-graph budget smoke before any repo queries, so a fresh worktree with no `bench/repos`
...
ignored** and locations are made **repo-relative**. Each query runs with `cwd` set to the
...
- `recommended_surface_counts` — family product placement counts for `default`,
  `divergence`, `hidden`, and reserved `debug`
...
- per family — **keyed by the normalized location set, not `family_id`** (so deliberate
  `family_id` scheme migrations and accidental ID regressions stay auditable):
...
- `fragment_kind_counts` / `reason_code_counts` — exact-fragment metadata buckets from
  product query JSON. #45 makes these live for current output, so fragment/reason drift is
...
These #51 C1 metrics are computed by the harness from stable query JSON. They do not add
new query JSON fields and do not change detector acceptance, ranking, or `top=0`
visibility.
...
- semantic `nose query ... all top=0 --mode semantic --format json` wall time;
...
NOSE_TIME=1 NOSE_TIME_UNITS=1 NOSE_TIME_UNIT_SUMMARY=1 \
  target/release/nose query <repo> all top=0 --mode semantic --format json \
...
```sh
python3 bench/type4/query_regression/query_regression.py cache --nose ./target/release/nose
```
...
The repo drift thresholds live in `THRESHOLDS` at the top of `query_regression.py`. Tune
...
small Go repo, all intended to stay sub-second for single-pass queries so the no-cache
...
Re-record `baseline.v1.json` when `main`'s product output legitimately changes (an
accepted detector change merges). Regenerate from `main` with the `baseline` command
