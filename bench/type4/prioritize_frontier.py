...
CANDIDATES = [
    ...
    Candidate(
        ...
        3,
        "partially-covered",
        "Static literal collection membership, typed dynamic receiver membership including Python `tuple[T, ...]`, Java `Queue<T>`, and Rust `VecDeque<T>` type facts, Python builtin `set`/`tuple`/`frozenset` factories, function-local Go slice / Java `List.of` / Rust `vec!` constructed bindings, proven Set construction, static JS/TS array `.some(...)` existential, `.every(...)` absence, `.indexOf(...)` membership comparisons, `.findIndex(...)` lambda membership comparisons, and `.filter(...).length` nonempty membership / zero-count absence checks, Java literal collection factories, same-file module/static-final JS/TS/Java collection bindings, Go `slices.Contains` over package-level proven slice bindings, and Rust local immutable literal array/slice bindings are covered; substring contains, map-key membership, mutated or append-expanded bindings, shadowed constructors/types/packages, untyped dynamic sets, and ambiguous receiver contains must stay distinct.",
        "Continue with dynamic collection/set membership only when receiver/element coordinates can be proven by imported or cross-file immutable bindings, construction facts, or type facts beyond the current typed-parameter, Python builtin factory, function-local constructed binding, Python tuple, Java Queue, Rust VecDeque, literal-Set, Java literal-factory, same-file module-binding, Go imported-package slice-binding, and Rust local literal-binding cases; keep substring, regex, map-key, mutation, shadowing, append-expanded construction, and unproven receiver-overloaded calls as hard boundaries.",
        (
    ...
    Candidate(
        ...
        4,
        "partially-covered",
        "Literal Python/Ruby map lookup, JS/TS inline/local/module Map and object defaults, typed Go/Java/Rust maps, typed TypeScript Map fallbacks, typed Python `dict`/`Mapping` fallbacks, Java Map.of/Map.ofEntries literal factories, Java static-final Map.of bindings, Rust std HashMap/BTreeMap literal factories, and Go `map[string]int|string|bool{...}[key]` zero-value default lookups are covered; cross-file imports, richer receiver facts, untyped Python/Ruby/JS receiver defaults, other Go zero values, absent-key semantics beyond proven zero defaults, and mutation/effects remain open.",
